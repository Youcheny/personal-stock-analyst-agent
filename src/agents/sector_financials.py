...
class FinancialsAnalyst:
    CHECKLIST = [
        "Underwriting/credit discipline (loss ratios, charge-offs)",
        "Capital adequacy & liquidity",
        "Cycle sensitivity (NIM, yield curve, underwriting standards)",
        "Regulatory risks & stress scenarios",
        "Fee vs spread revenue mix",
    ]
    ...
    def annotate(self, ticker: str) -> str:
        ...
        if "llm" in self.tools:
            try:
                ...
                return "### Financials Analyst Checklist\n\n" + "\n\n".join(analyses)
